...
"""
LatentAdditiveGET (Strategy B):
  X, p, cov + GET gene-specific correction:
      delta_gene[g] = Linear(GET_ct[g])

Memory-safe: does NOT expand (N × genes × d_get).
GET is indexed per celltype only.
"""
...
# =========================================
# 4️⃣ Convert expression matrices
# =========================================
def to_numpy(X):
...
# Convert GET matrices to tensor
get_list = []
...
assert n_genes_check == n_genes, "Gene count mismatch!"

print("Loaded GET tensor:", GET_tensor.shape)

# =========================================
# 8️⃣ Build DataLoaders (no GET inside)
# =========================================
...
# =========================================
# 9️⃣ Model: LatentAdditiveGET_B (Strategy B)
# =========================================
...
class LatentAdditiveGET_B(nn.Module):
    """
    Strategy B:
        base_pred[g] + Linear(GET_gene[g])
    """
    ...
    def __init__(
        ...
        # per-gene correction term from GET embedding
    ...
    def forward(self, x, p, cov_enc, cov_dec, ct_idx):
        ...
        base_out = self.decoder(torch.cat([latent_sum, cov_dec], dim=1))

        # GET gene mat for each cell
        GET_ct = self.GET[ct_idx]  # (batch × n_genes × d_get)
        gene_delta = self.linear_get(GET_ct).squeeze(-1)

        out = base_out + gene_delta
