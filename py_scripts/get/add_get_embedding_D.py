...
"""
LatentAdditiveGET_FiLM (Option: FiLM GET Modulation)

GET_ct → mean across genes → MLP → produce:
    gamma (latent_dim)
    beta  (latent_dim)

z_rna = encoder(x,p,cov)
z_mod = gamma * z_rna + beta

Final decoder reconstructs gene expression.

Memory-safe, no (cells × genes × d_get) expansion.
"""
...
ct_train_idx = torch.tensor([ct2idx[ct] for ct in train_adata.obs["celltype_mapped"].astype(str)],
                            dtype=torch.long)
ct_val_idx   = torch.tensor([ct2idx[ct] for ct in val_adata.obs["celltype_mapped"].astype(str)],
                            dtype=torch.long)
ct_test_idx  = torch.tensor([ct2idx[ct] for ct in test_adata.obs["celltype_mapped"].astype(str)],
                            dtype=torch.long)
...
# =========================================
# 9️⃣ Model: LatentAdditiveGET_FiLM
# =========================================
...
class LatentAdditiveGET_FiLM(nn.Module):
    """
    FiLM modulation:
        GET_ct → avg → MLP → gamma, beta
        z_mod = gamma * z_rna + beta
    """
    ...
    def __init__(
        ...
        get_tensor,
        latent_dim=160,
        film_dim=160,        # gamma,beta dimension (same as latent_dim)
        encoder_width=3072,
        n_layers=3,
        dropout=0.1,
        ...
        # FiLM GET encoder → outputs gamma and beta
        self.film_encoder = nn.Sequential(
            nn.Linear(d_get, encoder_width),
            nn.ReLU(),
            nn.Linear(encoder_width, 2 * film_dim)   # returns gamma || beta
        )

        # decoder consumes z_mod + cov_dec
        self.decoder = MLP(
    ...
    def forward(self, x, p, cov_enc, cov_dec, ct_idx):
        ...
        film_out = self.film_encoder(GET_avg)      # (batch × 2*latent_dim)
        gamma, beta = film_out.split(self.latent_dim, dim=1)

        # FiLM-modulated latent
        z_mod = gamma * z_rna + beta

        # decode
        out = self.decoder(torch.cat([z_mod, cov_dec], dim=1))
