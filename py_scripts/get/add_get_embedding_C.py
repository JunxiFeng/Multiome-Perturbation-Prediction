...
"""
LatentAdditiveGET_Encoder (Option 1):
    Adds a GET encoder:
        GET_ct  (genes × d_get)
          -> mean over genes: (d_get)
          -> MLP: GET_latent (latent_dim)

    Final latent:
        z_total = concat(z_rna, z_get, cov)

Memory-safe and celltype-aware.
"""
...
# =========================================
# 9️⃣ Model: LatentAdditiveGET_Encoder (Option 1)
# =========================================
...
class LatentAdditiveGET_Encoder(nn.Module):
    """
    Option 1 (GET Encoder):
        GET_ct → mean across genes → MLP → z_get
        final z_total = concat(z_rna, z_get, cov_dec)
    """
    ...
    def __init__(
        ...
        get_tensor,
        latent_dim=160,
        get_latent_dim=160,   # size of GET latent vector
        encoder_width=3072,
        n_layers=3,
        dropout=0.1,
        ...
        self.get_encoder = MLP(
            d_get,                  # input is mean over all genes
            encoder_width,
            get_latent_dim,         # output latent
            n_layers,
            dropout,
        )

        # decoder uses: latent_dim + get_latent_dim + n_covariates_dec
        self.decoder = MLP(
            latent_dim + get_latent_dim + n_covariates_dec,
            encoder_width,
            n_genes,
            n_layers,
            dropout,
    ...
    def forward(self, x, p, cov_enc, cov_dec, ct_idx):
        ...
        # Combine latents
        z_total = torch.cat([z_rna, z_get, cov_dec], dim=1)
...
model = LatentAdditiveGET_Encoder(
    n_genes=n_genes,
    n_perts=n_perts,
